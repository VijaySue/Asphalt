...
namespace Asphalt
{
    public partial class Weight : UserControl
    {
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            DateTime next = time.AddDays(1);
            string sql = string.Format("select * from weight where date(datetime) = date('{0}') order by name asc, batch asc;", time.ToString("yyyy-MM-dd"));
            ...
            if (reader.HasRows)
            {
                ...
                //根据查找的结果，动态生成左边菜单
                string[] gnames = new string[11] { "生产罐1", "生产罐2", "生产罐3", "生产罐4", "生产罐5", "生产罐6", "生产罐7", "生产罐8", "周转罐","预混罐1","预混罐2" };
        ...
        private void uiNavMenuEx1_MenuItemClick(TreeNode node, NavMenuItem item, int pageIndex)
        {
            try{
                ...
                DateTime time = dateTimePicker1.Value;
                DateTime next = time.AddDays(1);
                string sql = string.Format("select * from weight where name= '{0}' and batch= {1} and date(datetime) = date('{2}') order by name asc, batch asc;", name, batch, time.ToString("yyyy-MM-dd"));
                MySqlDataReader reader = Sql.executeReader(sql);
                //显示图表
                timer2.Stop();
                UILineOption option = new UILineOption();
                ...
                if (reader.Read())
                {
                    ...
                    option.YAxisScaleLines.Add(new UIScaleLine() { Color = Color.Red, Name = "上限", Value = reader.GetFloat(1) });
                    option.YAxisScaleLines.Add(new UIScaleLine() { Color = Color.Gold, Name = "下限", Value = reader.GetFloat(2) });

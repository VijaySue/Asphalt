...
namespace Asphalt
{
    public partial class Curve : UserControl
    {
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            string sql = string.Format("select * from temperature where date(datetime) = date('{0}') order by name asc, batch asc;", time.ToString("yyyy-MM-dd"));
        ...
        private void uiNavMenuEx1_MenuItemClick(TreeNode node, NavMenuItem item, int pageIndex)
        {
            try {
                ...
                DateTime time = dateTimePicker1.Value;
                string sql = string.Format("select * from temperature where name= '{0}' and batch= {1} and date(datetime) = date('{2}') order by name asc, batch asc;", name, batch, time.ToString("yyyy-MM-dd"));
                MySqlDataReader reader = Sql.executeReader(sql);
                //显示图表
                timer2.Stop();
                UILineOption option = new UILineOption();
